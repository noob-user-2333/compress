...
private struct decodeTableItem =
    let 
...
type tANS(initFreqArray:int array,fieldLen:int) =
    let initArray = if initFreqArray.Length <> (1 <<< fieldLen) ||
                            ((fieldLen - 1) &&& fieldLen) <> 0
                       then
                           raise (ArgumentException("控制字段状态数量必须为2的控制字段bit数次"))
                       else
                           initFreqArray
    let initInterval = if initArray.Sum() < updateInterval then
                                initArray.Sum()
                            else
    ...
    member this.Encode(controlField:int) =
        1

    member this.Decode(n: int) =
        1
