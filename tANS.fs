...
type private Item =
    struct
        ...
        new(sym: int, readBits: int, newX: int) =
            { symbol = sym
              readBits = readBits
              newX = newX }
    end
...
type private Decoder(freqArray: int array, spreadArray: int array, fieldLen: int) =
    ...
    do update freqArray spreadArray table

    member this.Decode(status: int, r: BitReaderFromEnd, [<Out>] nextStatus: byref<int>) =
        let item = table[status]
        let sym = item.symbol
        let needReadBits = item.readBits

        let bits =
            if needReadBits > 0 then
                let num = r.ReadBits(item.readBits)
                num
            else
                0UL
    ...
    member this.Update(freqArray: int array, spreadArray: int array) = update freqArray spreadArray table
...
type private Encoder(freqArray: int array, spreadArray: int array, fieldLen: int) =
    ...
    member this.Update(freqArray: int array, spreadArray: int array) = update freqArray spreadArray
//核心功能
...
type tANS(freqArray: int array, fieldLen: int) =
    ...
    member this.Update(freqArray: int array) = update freqArray
