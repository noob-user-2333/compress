...
type Gorilla() =
    member this.compress(writer:BitWriter,ptrBuffer:PtrBuffer) =
    ...
    interface ICompressor with
        member _.GetName() =
            "Gorilla"
        member this.Compress (writer, ptrBuffer) = 
            this.compress (writer,ptrBuffer)
        
        
        /// 解压缩
        member _.Decompress (r:BitReader,length: int) : double[] =
